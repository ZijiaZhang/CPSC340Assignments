...
\\ f'(w) = \frac{1}{2} w^TX^TVX + \frac{1}{2} w^T (X^TVX)^T - y^TVX + 0 + \lambda u^T
\\ f'(w) = w^TX^TVX - y^TVX + \lambda u^T$
Therefore, the $ w = (X^TVX)^2 \setminus ((X^TVX)(X^TVy- \lambda u)) $
}
...
\ans{The minimum is 5, and the Test Error is 239.5448550643697}
